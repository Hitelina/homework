...
def test_filter_by_state_executed(test_data: List[Dict]):
...
def test_filter_by_state_canceled(test_data: List[Dict]):
...
def test_filter_by_state_no_match(test_data: List[Dict]):
    """Тестирование фильтрации, когда нет совпадений."""
    filtered_data = filter_by_state(test_data, state="UNKNOWN")
    assert len(filtered_data) == 0


def test_sort_by_date_ascending(test_data: List[Dict]):
...
def test_sort_by_date_descending(test_data: List[Dict]):
